...
import cc.toprank.byd.common.utils.validate.ValidateUtils;
...
public class StringUtil extends StringUtilParent {
    ...
    public static String replaceMobile(String mobile) {
        if (StringUtils.isNotBlank(mobile) && ValidateUtils.isMobile(mobile)) {
            String newMobile = mobile.substring(0, mobile.length() - (mobile.substring(3)).length()) + "****" + mobile.substring(7);
            return newMobile;
        }
        return "";
    }
    ...
    public static String replaceEmail(String email) {
        if (StringUtils.isNotBlank(email) && ValidateUtils.isEmail(email)) {
            String[] str = email.split("@");
            return str[0].charAt(0) + "****@" + str[1];
        } else {
            return email;
        }
    }

    public static String cutoffLastExcision(String str, String excision) {
        if (!(str == null || "".equals(str)) && !str.equals("0")) {
            int index = str.lastIndexOf(excision);
